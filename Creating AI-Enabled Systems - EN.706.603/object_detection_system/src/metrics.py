...
class Loss:
    ...
    def compute(self, predictions, annotations):
        ...
        pred_box = np.array([preds[:4] for preds in itertools.chain(*predictions)])
        pred_score = np.array([preds[5:] for preds in itertools.chain(*predictions)])
        objectness_score = np.array([preds[4] for preds in itertools.chain(*predictions)])

        gt_box = np.array([anns[1:] for anns in annotations])
        gt_class_id = np.array([anns[0] for anns in annotations])

        ious = Metrics.calculate_ious(np.vstack([gt_box, pred_box]))[:len(annotations), len(annotations):]
        ...
        for idx, (obj_mask, noobj_mask) in enumerate(zip(obj_masks, noobj_masks)):        
            loc_loss += self.lambda_coord * np.sum((pred_box[obj_mask] - gt_box[idx][0:4]) ** 2) 
            conf_loss_obj += np.sum((objectness_score[obj_mask] - 1) ** 2)
            conf_loss_noobj += self.lambda_noobj * np.sum((objectness_score[noobj_mask] - 0) ** 2)
            class_loss += self.__cross_entropy_loss(
                (np.arange(self.num_classes) == gt_class_id[idx]).astype(float), pred_score[obj_mask]
            )
...
if __name__ == "__main__":
    ...
    y_true = np.random.randint(0, 3, 3000)  # Actual labels for 3000 samples with 3 classes (0, 1, 2)
    scores = np.random.rand(3000, 3)  # Random predicted scores/probabilities for each class
    num_classes = 3
